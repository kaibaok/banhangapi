...
class DeskTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('desk', function (Blueprint $table) {
            $table->id();
            $table->string('name');
            $table->dateTime('used_at')->default(Carbon::now()->toDateTimeString())->nullable(true);
            $table->integer('status')->default(config('constants.desk_status.avaliable'));
            $table->timestamps();
        });

        DB::table('desk')->insert([
            [
                'id' => 1,
                'name' => 'B.1',
